...
def footer():
    ...
    ''')
    with ui.element('footer').classes('flex items-center justify-between w-full h-16 px-4 text-white').style('background-color: #1f2937 !important; color: white !important;'):
        # Logo section
        with ui.row().classes('items-center gap-3'):
            ui.icon('hub', size='1.5rem').style('color: #0055B8 !important;')
            ui.label('TalentConnect').classes('text-lg font-semibold text-white').style('font-family: "Raleway", sans-serif;')
        ui.label('© 2025 TalentConnect. All rights reserved.').classes('text-sm')
